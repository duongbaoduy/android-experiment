...
#include "audio_device/audio_device_utility.h"
...
int32_t AudioRecordJni::SetAndroidAudioDeviceObjects(void* javaVM, void* env,
	...
	globalScClass = reinterpret_cast<jclass>(globalJNIEnv->NewGlobalRef(
			javaScClassLocal));
	if (!globalScClass) {
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, -1,
...
AudioRecordJni::AudioRecordJni(const int32_t id,
		PlayoutDelayProvider* delay_provider) :
		_javaVM(NULL), _jniEnvRec(NULL), _javaScClass(0), _javaScObj(0), _javaRecBuffer(
				0), _javaDirectRecBuffer(NULL), _javaMidRecAudio(0), _ptrAudioBuffer(
				NULL), _critSect(
				*CriticalSectionWrapper::CreateCriticalSection()), _id(id), _delay_provider(
				delay_provider), _initialized(false), _timeEventRec(
				*EventWrapper::Create()), _recStartStopEvent(
				*EventWrapper::Create()), _ptrThreadRec(NULL), _recThreadID(0), _recThreadIsInitialized(
				false), _shutdownRecThread(false), _recordingDeviceIsSpecified(
				false), _recording(false), _recIsInitialized(false), _micIsInitialized(
				false), _startRec(false), _recWarning(0), _recError(0), _delayRecording(
				0), _AGC(false), _samplingFreqIn(
				(N_REC_SAMPLES_PER_SEC / 1000)), _recAudioSource(1) { // 1 is AudioSource.MIC which is our default
...
int32_t AudioRecordJni::Terminate() {
	...
	if (_javaVM->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
		...
		if ((res < 0) || !env) {
			WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
					"%s: Could not attach thread to JVM (%d, %p)", __FUNCTION__,
					res, env);
			return -1;
		}
		isAttached = true;
...
int32_t AudioRecordJni::MicrophoneVolumeIsAvailable(bool& available) { // NOLINT
...
int32_t AudioRecordJni::MaxMicrophoneVolume(uint32_t& maxVolume) const { // NOLINT
	WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
			"  API call not supported on this platform");
	return -1;
}

int32_t AudioRecordJni::MinMicrophoneVolume(uint32_t& minVolume) const { // NOLINT
	WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
			"  API call not supported on this platform");
	return -1;
}

int32_t AudioRecordJni::MicrophoneVolumeStepSize(uint16_t& stepSize) const {
	WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
			"  API call not supported on this platform");
	return -1;
}
...
bool AudioRecordJni::BuiltInAECIsAvailable() const {
	...
	jmethodID builtInAECIsAvailable = env->GetStaticMethodID(_javaScClass,
			"BuiltInAECIsAvailable", "()Z");
...
int32_t AudioRecordJni::InitJavaResources() {
	...
	WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id, "get method id");
	...
	WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id, "construct object",
			__FUNCTION__);
	...
	if (!_javaScObj) {
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
				"%s: could not create Java sc object reference", __FUNCTION__);
		return -1;
	}
	...
	if (globalContext) {
		jfieldID context_id = env->GetFieldID(globalScClass, "_context",
...
int32_t AudioRecordJni::InitSampleRate() {
	...
	if (_samplingFreqIn > 0) {
		...
		WEBRTC_TRACE(kTraceStateInfo, kTraceAudioDevice, _id,
				"  Trying configured recording sampling rate %d", samplingFreq);
	}
	...
	while (keepTrying) {
		...
		if (res < 0) {
			...
			default: // error
				WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
						"%s: InitRecording failed (%d)", __FUNCTION__, res);
				return -1;
			}
		} else {
			keepTrying = false;
...
bool AudioRecordJni::RecThreadProcess() {
	if (!_recThreadIsInitialized) {
		...
		if ((res < 0) || !_jniEnvRec) {
			WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
					"Could not attach rec thread to JVM (%d, %p)", res,
					_jniEnvRec);
	...
	if (!_recording) {
		...
		case kEventSignaled:
			WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
					"Recording thread event signal");
			_timeEventRec.Reset();
			break;
		case kEventError:
			WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
					"Recording thread event error");
			return true;
		case kEventTimeout:
			WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
					"Recording thread event timeout");
			return true;
		}
	}
	...
	if (_recording) {
		...
		jint recDelayInSamples = _jniEnvRec->CallIntMethod(_javaScObj,
				_javaMidRecAudio, 2 * samplesToRec);
	...
	if (_shutdownRecThread) {
		...
		if (_javaVM->DetachCurrentThread() < 0) {
			WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
					"Could not detach recording thread from JVM");
