...
#include "audio_device/android/opensles_input.h"
...
OpenSlesInput::OpenSlesInput(const int32_t id,
		PlayoutDelayProvider* delay_provider) :
		id_(id), delay_provider_(delay_provider), initialized_(false), mic_initialized_(
				false), rec_initialized_(false), crit_sect_(
				CriticalSectionWrapper::CreateCriticalSection()), recording_(
				false), num_fifo_buffers_needed_(0), number_overruns_(0), sles_engine_(
				NULL), sles_engine_itf_(NULL), sles_recorder_(NULL), sles_recorder_itf_(
				NULL), sles_recorder_sbq_itf_(NULL), audio_buffer_(NULL), active_queue_(
				0), rec_sampling_rate_(0), agc_enabled_(false), recording_delay_(
				0) {
}
...
int32_t OpenSlesInput::SetAndroidAudioDeviceObjects(void* javaVM, void* env,
		void* context) {
	return 0;
}
...
int32_t OpenSlesInput::StartRecording() {
	...
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_sbq_itf_)->RegisterCallback( sles_recorder_sbq_itf_, RecorderSimpleBufferQueueCallback, this),
			-1);
...
int32_t OpenSlesInput::MinMicrophoneVolume(uint32_t& minVolume) const { // NOLINT
	minVolume = 0;
	return 0;
}

int32_t OpenSlesInput::MicrophoneVolumeStepSize(uint16_t& stepSize) const {
	stepSize = 1;
	return 0;
}
...
void OpenSlesInput::UpdateRecordingDelay() {
	...
	int outstanding_samples = (TotalBuffersUsed() - 0.5)
			* buffer_size_samples();
...
void OpenSlesInput::UpdateSampleRate() {
	rec_sampling_rate_ =
			audio_manager_.low_latency_supported() ?
...
bool OpenSlesInput::EnqueueAllBuffers() {
	...
	for (int i = 0; i < kNumOpenSlBuffers; ++i) {
		...
		OPENSL_RETURN_ON_FAILURE(
				(*sles_recorder_sbq_itf_)->Enqueue( sles_recorder_sbq_itf_, reinterpret_cast<void*>(rec_buf_[i].get()), buffer_size_bytes()),
				false);
	}
	...
	assert(fifo_->size() == fifo_->capacity() || fifo_->size() == 0);
...
bool OpenSlesInput::CreateAudioRecorder() {
	...
	SLDataLocator_IODevice micLocator = { SL_DATALOCATOR_IODEVICE,
	...
	const SLInterfaceID id[kNumInterfaces] = { SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
			SL_IID_ANDROIDCONFIGURATION };
	const SLboolean req[kNumInterfaces] = { SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE };
	OPENSL_RETURN_ON_FAILURE(
			(*sles_engine_itf_)->CreateAudioRecorder(sles_engine_itf_, &sles_recorder_, &audio_source, &audio_sink, kNumInterfaces, id, req),
			false);

	SLAndroidConfigurationItf recorder_config;
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_)->GetInterface(sles_recorder_, SL_IID_ANDROIDCONFIGURATION, &recorder_config),
			false);
	...
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_)->Realize(sles_recorder_, SL_BOOLEAN_FALSE),
			false);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_)->GetInterface(sles_recorder_, SL_IID_RECORD, static_cast<void*>(&sles_recorder_itf_)),
			false);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_)->GetInterface( sles_recorder_, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, static_cast<void*>(&sles_recorder_sbq_itf_)),
			false);
	return true;
}
...
bool OpenSlesInput::HandleOverrun(int event_id, int event_msg) {
	...
	// is set to NULL.
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_itf_)->SetRecordState(sles_recorder_itf_, SL_RECORDSTATE_STOPPED),
			true);
	EnqueueAllBuffers();
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_itf_)->SetRecordState(sles_recorder_itf_, SL_RECORDSTATE_RECORDING),
			true);
	return true;
}

void OpenSlesInput::RecorderSimpleBufferQueueCallback(
		SLAndroidSimpleBufferQueueItf queue_itf, void* context) {
...
void OpenSlesInput::RecorderSimpleBufferQueueCallbackHandler(
	...
	int next_free_buffer = (active_queue_ + kNumOpenSlBuffers - 1)
			% TotalBuffersUsed();
	OPENSL_RETURN_ON_FAILURE(
			(*sles_recorder_sbq_itf_)->Enqueue( sles_recorder_sbq_itf_, reinterpret_cast<void*>(rec_buf_[next_free_buffer].get()), buffer_size_bytes()),
			VOID_RETURN);
}
