...
#include "audio_device/android/opensles_output.h"

#include <assert.h>

#include "audio_device/android/opensles_common.h"
#include "audio_device/android/fine_audio_buffer.h"
#include "audio_device/android/single_rw_fifo.h"
...
static const SLEngineOption kOption[] = { { SL_ENGINEOPTION_THREADSAFE,
		static_cast<SLuint32>(SL_BOOLEAN_TRUE) }, };

enum {
	kNoUnderrun, kUnderrun,
};

namespace webrtc {

OpenSlesOutput::OpenSlesOutput(const int32_t id) :
		id_(id), initialized_(false), speaker_initialized_(false), play_initialized_(
				false), crit_sect_(
				CriticalSectionWrapper::CreateCriticalSection()), playing_(
				false), num_fifo_buffers_needed_(0), number_underruns_(0), sles_engine_(
				NULL), sles_engine_itf_(NULL), sles_player_(NULL), sles_player_itf_(
				NULL), sles_player_sbq_itf_(NULL), sles_output_mixer_(NULL), audio_buffer_(
				NULL), active_queue_(0), speaker_sampling_rate_(
				kDefaultSampleRate), buffer_size_samples_(0), buffer_size_bytes_(
				0), playout_delay_(0) {
}
...
int32_t OpenSlesOutput::SetAndroidAudioDeviceObjects(void* javaVM, void* env,
		void* context) {
...
int32_t OpenSlesOutput::Init() {
	...
	OPENSL_RETURN_ON_FAILURE(
			slCreateEngine(&sles_engine_, 1, kOption, 0, NULL, NULL), -1);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_engine_)->Realize(sles_engine_, SL_BOOLEAN_FALSE), -1);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_engine_)->GetInterface(sles_engine_, SL_IID_ENGINE, &sles_engine_itf_),
			-1);
	// Set up OpenSl output mix.
	OPENSL_RETURN_ON_FAILURE(
			(*sles_engine_itf_)->CreateOutputMix(sles_engine_itf_, &sles_output_mixer_, 0, NULL, NULL),
			-1);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_output_mixer_)->Realize(sles_output_mixer_, SL_BOOLEAN_FALSE),
			-1);
...
int32_t OpenSlesOutput::StartPlayout() {
	...
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_sbq_itf_)->RegisterCallback(sles_player_sbq_itf_, PlayerSimpleBufferQueueCallback, this),
			-1);
...
int32_t OpenSlesOutput::SpeakerVolumeStepSize(uint16_t& stepSize) const { // NOLINT
	assert(speaker_initialized_);
	stepSize = 1;
	return 0;
}
...
int32_t OpenSlesOutput::PlayoutBuffer(AudioDeviceModule::BufferType& type, // NOLINT
...
void OpenSlesOutput::CalculateNumFifoBuffersNeeded() {
	int number_of_bytes_needed = (speaker_sampling_rate_ * kNumChannels
			* sizeof(int16_t)) * 10 / 1000;

	// Ceiling of integer division: 1 + ((x - 1) / y)
	int buffers_per_10_ms = 1
			+ ((number_of_bytes_needed - 1) / buffer_size_bytes_);
...
void OpenSlesOutput::AllocateBuffers() {
	...
	fine_buffer_.reset(
			new FineAudioBuffer(audio_buffer_, buffer_size_bytes_,
...
bool OpenSlesOutput::EnqueueAllBuffers() {
	...
	for (int i = 0; i < kNumOpenSlBuffers; ++i) {
		...
		OPENSL_RETURN_ON_FAILURE(
				(*sles_player_sbq_itf_)->Enqueue( sles_player_sbq_itf_, reinterpret_cast<void*>(play_buf_[i].get()), buffer_size_bytes_),
				false);
	}
...
bool OpenSlesOutput::CreateAudioPlayer() {
	...
	SLDataFormat_PCM configuration = webrtc_opensl::CreatePcmConfiguration(
			speaker_sampling_rate_);
	...
	SLInterfaceID ids[kNumInterfaces] = { SL_IID_BUFFERQUEUE, SL_IID_VOLUME,
			SL_IID_ANDROIDCONFIGURATION };
	SLboolean req[kNumInterfaces] = { SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE,
			SL_BOOLEAN_TRUE };
	OPENSL_RETURN_ON_FAILURE(
			(*sles_engine_itf_)->CreateAudioPlayer(sles_engine_itf_, &sles_player_, &audio_source, &audio_sink, kNumInterfaces, ids, req),
			false);

	SLAndroidConfigurationItf player_config;
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_)->GetInterface(sles_player_, SL_IID_ANDROIDCONFIGURATION, &player_config),
			false);
	...
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_)->Realize(sles_player_, SL_BOOLEAN_FALSE), false);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_)->GetInterface(sles_player_, SL_IID_PLAY, &sles_player_itf_),
			false);
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_)->GetInterface(sles_player_, SL_IID_BUFFERQUEUE, &sles_player_sbq_itf_),
			false);
	return true;
}
...
bool OpenSlesOutput::HandleUnderrun(int event_id, int event_msg) {
	...
	// set to NULL.
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_itf_)->SetPlayState(sles_player_itf_, SL_PLAYSTATE_STOPPED),
			true);
	EnqueueAllBuffers();
	OPENSL_RETURN_ON_FAILURE(
			(*sles_player_itf_)->SetPlayState(sles_player_itf_, SL_PLAYSTATE_PLAYING),
			true);
	return true;
}

void OpenSlesOutput::PlayerSimpleBufferQueueCallback(
		SLAndroidSimpleBufferQueueItf sles_player_sbq_itf, void* p_context) {
...
void OpenSlesOutput::PlayerSimpleBufferQueueCallbackHandler(
	...
	if (audio)
		OPENSL_RETURN_ON_FAILURE(
				(*sles_player_sbq_itf)->Enqueue(sles_player_sbq_itf, audio, buffer_size_bytes_),
				VOID_RETURN);
...
bool OpenSlesOutput::StartCbThreads() {
	play_thread_.reset(
			ThreadWrapper::CreateThread(CbThread, this, kRealtimePriority,
					"opensl_play_thread"));
