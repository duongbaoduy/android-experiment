...
public:
	static int32_t SetAndroidAudioDeviceObjects(void* javaVM, void* env,
			void* context) {
		if (OutputType::SetAndroidAudioDeviceObjects(javaVM, env, context)
				== -1) {
			return -1;
		}
	...
	explicit AudioDeviceTemplate(const int32_t id) :
			output_(id), input_(id, &output_) {
	}

	virtual ~AudioDeviceTemplate() {
	}

	int32_t ActiveAudioLayer(AudioDeviceModule::AudioLayer& audioLayer) const { // NOLINT
	...
	int32_t WaveOutVolume(uint16_t& volumeLeft, // NOLINT
	...
	int32_t MicrophoneVolumeStepSize(uint16_t& stepSize) const { // NOLINT
	...
	int32_t CPULoad(uint16_t& load) const { // NOLINT
		WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, 0,
				"  API call not supported on this platform");
		return -1;
	}
	...
	void ClearPlayoutWarning() {
		output_.ClearPlayoutWarning();
	}

	void ClearPlayoutError() {
		output_.ClearPlayoutError();
	}

	void ClearRecordingWarning() {
		input_.ClearRecordingWarning();
	}

	void ClearRecordingError() {
		input_.ClearRecordingError();
	}
