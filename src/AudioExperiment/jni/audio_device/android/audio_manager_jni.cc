...
#include "audio_device/android/audio_manager_jni.h"
...
#define THIS_FILE "audio_manager_jni.cc"
...
AudioManagerJni::AudioManagerJni() :
		low_latency_supported_(false), native_output_sample_rate_(0), native_buffer_size_(
				0) {
	if (!HasDeviceObjects()) {
		assert(false);
	}
...
void AudioManagerJni::SetAndroidAudioDeviceObjects(void* jvm, void* env,
	...
	g_audio_manager_class_ = reinterpret_cast<jclass>(g_jni_env_->NewGlobalRef(
			javaAmClassLocal));
...
void AudioManagerJni::SetNativeFrameSize(JNIEnv* env) {
	jmethodID id = LookUpMethodId(env, "getAudioLowLatencyOutputFrameSize",
			"()I");
...
jmethodID AudioManagerJni::LookUpMethodId(JNIEnv* env, const char* method_name,
...
void AudioManagerJni::CreateInstance(JNIEnv* env) {
	...
	jmethodID id = LookUpMethodId(env, "<init>",
