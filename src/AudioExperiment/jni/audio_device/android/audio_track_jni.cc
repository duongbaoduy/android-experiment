...
AudioTrackJni::AudioTrackJni(const int32_t id) :
		_javaVM(NULL), _jniEnvPlay(NULL), _javaScClass(0), _javaScObj(0), _javaPlayBuffer(
				0), _javaDirectPlayBuffer(NULL), _javaMidPlayAudio(0), _ptrAudioBuffer(
				NULL), _critSect(
				*CriticalSectionWrapper::CreateCriticalSection()), _id(id), _initialized(
				false), _timeEventPlay(*EventWrapper::Create()), _playStartStopEvent(
				*EventWrapper::Create()), _ptrThreadPlay(NULL), _playThreadID(
				0), _playThreadIsInitialized(false), _shutdownPlayThread(false), _playoutDeviceIsSpecified(
				false), _playing(false), _playIsInitialized(false), _speakerIsInitialized(
				false), _startPlay(false), _playWarning(0), _playError(0), _delayPlayout(
				0), _samplingFreqOut((N_PLAY_SAMPLES_PER_SEC / 1000)), _maxSpeakerVolume(
				0) {
}
...
int32_t AudioTrackJni::InitPlayout() {
	...
	// get the JNI env for this thread
	if (_javaVM->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
		WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id, "attaching");
	...
	// Detach this thread if it was attached
	if (isAttached) {
		WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id, "detaching");
		if (_javaVM->DetachCurrentThread() < 0) {
			WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
...
int32_t AudioTrackJni::SpeakerVolumeStepSize(uint16_t& stepSize) const { // NOLINT
	if (!_speakerIsInitialized) {
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
				"  Speaker not initialized");
		return -1;
...
int32_t AudioTrackJni::PlayoutBuffer(AudioDeviceModule::BufferType& type, // NOLINT
...
int32_t AudioTrackJni::InitJavaResources() {
	...
	jobject javaPlayBufferLocal = env->GetObjectField(_javaScObj,
			fidPlayBuffer);
...
int32_t AudioTrackJni::InitSampleRate() {
	...
	if (_samplingFreqOut > 0) {
		...
		WEBRTC_TRACE(kTraceStateInfo, kTraceAudioDevice, _id,
				"  Trying configured playback sampling rate %d", samplingFreq);
	} else {
...
bool AudioTrackJni::PlayThreadProcess() {
	if (!_playThreadIsInitialized) {
		...
		if ((res < 0) || !_jniEnvPlay) {
			WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
					"Could not attach playout thread to JVM (%d, %p)", res,
					_jniEnvPlay);
	...
	if (!_playing) {
		...
		case kEventSignaled:
			WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
					"Playout thread event signal");
			_timeEventPlay.Reset();
			break;
		case kEventError:
			WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
					"Playout thread event error");
			return true;
		case kEventTimeout:
			WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id,
					"Playout thread event timeout");
			return true;
		}
	}
	...
	if (_startPlay) {
		...
		WEBRTC_TRACE(kTraceDebug, kTraceAudioDevice, _id, "Sent signal");
	}
	...
	if (_playing) {
		...
		// audio-thread lock
		UnLock();
		uint32_t nSamples = _ptrAudioBuffer->RequestPlayoutData(samplesToPlay);
		Lock();
	...
	if (_shutdownPlayThread) {
		...
		if (_javaVM->DetachCurrentThread() < 0) {
			WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
					"Could not detach playout thread from JVM");
