...
int kPlayoutBufferSize = kPlayoutFixedSampleRate / 100 * kPlayoutNumChannels
		* 2;
int kRecordingBufferSize = kRecordingFixedSampleRate / 100
		* kRecordingNumChannels * 2;

FileAudioDevice::FileAudioDevice(const int32_t id, const char* inputFilename,
		const char* outputFile) :
		_ptrAudioBuffer(NULL), _recordingBuffer(NULL), _playoutBuffer(NULL), _recordingFramesLeft(
				0), _playoutFramesLeft(0), _critSect(
				*CriticalSectionWrapper::CreateCriticalSection()), _recordingBufferSizeIn10MS(
				0), _recordingFramesIn10MS(0), _playoutFramesIn10MS(0), _ptrThreadRec(
				NULL), _ptrThreadPlay(NULL), _recThreadID(0), _playThreadID(0), _playing(
				false), _recording(false), _lastCallPlayoutMillis(0), _lastCallRecordMillis(
				0), _outputFile(*FileWrapper::Create()), _inputFile(
				*FileWrapper::Create()), _outputFilename(outputFile), _inputFilename(
				inputFilename), _clock(Clock::GetRealTimeClock()) {
}
...
int32_t FileAudioDevice::StartPlayout() {
	...
	if (!_playoutBuffer)
		_playoutBuffer = new int8_t[2 * kPlayoutNumChannels
	...
	if (_outputFile.OpenFile(_outputFilename.c_str(), false, false, false)
			== -1) {
...
int32_t FileAudioDevice::StartRecording() {
	...
	if (_inputFile.OpenFile(_inputFilename.c_str(), true, true, false) == -1) {
