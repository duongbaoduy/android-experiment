...
#include "audio_device/dummy/audio_device_utility_dummy.h"
...
AudioDeviceModule* CreateAudioDeviceModule(int32_t id,
...
AudioDeviceModule* AudioDeviceModuleImpl::Create(const int32_t id,
	...
	RefCountImpl<AudioDeviceModuleImpl>* audioDevice = new RefCountImpl<
	...
	// dbd: fix me when add audio processing
//	WebRtcSpl_Init();
...
AudioDeviceModuleImpl::AudioDeviceModuleImpl(const int32_t id,
		const AudioLayer audioLayer) :
		_critSect(*CriticalSectionWrapper::CreateCriticalSection()), _critSectEventCb(
				*CriticalSectionWrapper::CreateCriticalSection()), _critSectAudioCb(
				*CriticalSectionWrapper::CreateCriticalSection()), _ptrCbAudioDeviceObserver(
				NULL), _ptrAudioDeviceUtility(NULL), _ptrAudioDevice(NULL), _id(
				id), _platformAudioLayer(audioLayer), _lastProcessTime(
				AudioDeviceUtility::GetTimeInMS()), _platformType(
				kPlatformNotSupported), _initialized(false), _lastError(
				kAdmErrNone) {
...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects() {
	WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "dbd - %s", __FUNCTION__);
...
int32_t AudioDeviceModuleImpl::SetPlayoutBuffer(const BufferType type,
	...
	if (_ptrAudioDevice->PlayoutIsInitialized()) {
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
				"unable to modify the playout buffer while playing side is initialized");
		return -1;
	}
