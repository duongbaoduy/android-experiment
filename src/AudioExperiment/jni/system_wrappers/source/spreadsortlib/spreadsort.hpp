...
inline size_t get_max_count(unsigned log_range, size_t count) {
...
inline int get_log_divisor(size_t count, unsigned log_range) {
	int log_divisor;
...
size_bins(std::vector<size_t> &bin_sizes,
		std::vector<RandomAccessIter> &bin_cache, unsigned cache_offset,
		unsigned &cache_end, unsigned bin_count) {
...
inline void spread_sort_rec(RandomAccessIter first, RandomAccessIter last,
		std::vector<RandomAccessIter> &bin_cache, unsigned cache_offset,
	...
	unsigned log_divisor = get_log_divisor(last - first,
			rough_log_2_size((size_t) (*max >> 0) - (*min >> 0)));
	...
	for (unsigned u = 0; u < bin_count - 1; ++u) {
		...
		for (RandomAccessIter current = *local_bin; current < nextbinstart;
			...
			for (target_bin = (bins + ((*current >> log_divisor) - div_min));
				...
				RandomAccessIter * b_bin = bins
						+ ((*b >> log_divisor) - div_min);
				if (b_bin != local_bin) {
	...
	for (unsigned u = cache_offset; u < cache_end;
		...
		if (count < max_count)
			...
			spread_sort_rec<RandomAccessIter, div_type, data_type>(lastPos,
					bin_cache[u], bin_cache, cache_end, bin_sizes);
	}
}
...
inline void inner_swap_loop(RandomAccessIter * bins,
		const RandomAccessIter & nextbinstart, unsigned ii, right_shift &shift,
		const unsigned log_divisor, const div_type div_min) {
	RandomAccessIter * local_bin = bins + ii;
	for (RandomAccessIter current = *local_bin; current < nextbinstart;
			++current) {
		for (RandomAccessIter * target_bin = (bins
				+ (shift(*current, log_divisor) - div_min));
				target_bin != local_bin;
				target_bin = bins + (shift(*current, log_divisor) - div_min)) {
			data_type tmp;
			RandomAccessIter b = (*target_bin)++;
			RandomAccessIter * b_bin = bins
					+ (shift(*b, log_divisor) - div_min);
...
template<class RandomAccessIter, class div_type, class data_type,
		class right_shift>
inline void swap_loop(RandomAccessIter * bins, RandomAccessIter & nextbinstart,
		unsigned ii, right_shift &shift, const std::vector<size_t> &bin_sizes,
		const unsigned log_divisor, const div_type div_min) {
	nextbinstart += bin_sizes[ii];
	inner_swap_loop<RandomAccessIter, div_type, data_type, right_shift>(bins,
			nextbinstart, ii, shift, log_divisor, div_min);
}

//Functor implementation for recursive sorting
template<class RandomAccessIter, class div_type, class data_type,
		class right_shift, class compare>
inline void spread_sort_rec(RandomAccessIter first, RandomAccessIter last,
		std::vector<RandomAccessIter> &bin_cache, unsigned cache_offset,
	...
	if (max == min)
		return;
	unsigned log_divisor = get_log_divisor(last - first,
			rough_log_2_size((size_t) (shift(*max, 0)) - (shift(*min, 0))));
	...
	RandomAccessIter nextbinstart = first;
	for (unsigned u = 0; u < bin_count - 1; ++u)
		swap_loop<RandomAccessIter, div_type, data_type, right_shift>(bins,
				nextbinstart, u, shift, bin_sizes, log_divisor, div_min);
	bins[bin_count - 1] = last;
	...
	for (unsigned u = cache_offset; u < cache_end;
		...
		if (count < max_count)
			...
			spread_sort_rec<RandomAccessIter, div_type, data_type, right_shift,
					compare>(lastPos, bin_cache[u], bin_cache, cache_end,
					bin_sizes, shift, comp);
	}
}
...
inline void spread_sort_rec(RandomAccessIter first, RandomAccessIter last,
	...
	for (unsigned ii = 0; ii < bin_count - 1; ++ii)
		swap_loop<RandomAccessIter, div_type, data_type, right_shift>(bins,
				nextbinstart, ii, shift, bin_sizes, log_divisor, div_min);
	bins[bin_count - 1] = last;
...
template<class RandomAccessIter, class div_type, class data_type>
inline void spread_sort(RandomAccessIter first, RandomAccessIter last, div_type,
		data_type) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	spread_sort_rec<RandomAccessIter, div_type, data_type>(first, last,
...
template<class RandomAccessIter, class div_type, class data_type,
		class right_shift, class compare>
inline void spread_sort(RandomAccessIter first, RandomAccessIter last, div_type,
		data_type, right_shift shift, compare comp) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	spread_sort_rec<RandomAccessIter, div_type, data_type, right_shift, compare>(
...
template<class RandomAccessIter, class div_type, class data_type,
		class right_shift>
inline void spread_sort(RandomAccessIter first, RandomAccessIter last, div_type,
		data_type, right_shift shift) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	spread_sort_rec<RandomAccessIter, div_type, data_type, right_shift>(first,
			last, bin_cache, 0, bin_sizes, shift);
}
}
...
inline cast_type mem_cast(const data_type & data) {
	cast_type result;
...
inline void find_extremes(RandomAccessIter current, RandomAccessIter last,
		div_type & max, div_type & min, right_shift shift) {
...
inline void inner_float_swap_loop(RandomAccessIter * bins,
	...
	for (RandomAccessIter current = *local_bin; current < nextbinstart;
		...
		for (RandomAccessIter * target_bin = (bins
			...
			RandomAccessIter b = (*target_bin)++;
			RandomAccessIter * b_bin = bins
					+ ((cast_float_iter<div_type, RandomAccessIter>(b)
							>> log_divisor) - div_min);
			//Three-way swap; if the item to be swapped doesn't belong in the current bin, swap it to where it belongs
			if (b_bin != local_bin) {
				RandomAccessIter c = (*b_bin)++;
...
inline void float_swap_loop(RandomAccessIter * bins,
		...
		const std::vector<size_t> &bin_sizes, const unsigned log_divisor,
		const div_type div_min) {
	nextbinstart += bin_sizes[ii];
	inner_float_swap_loop<RandomAccessIter, div_type, data_type>(bins,
			nextbinstart, ii, log_divisor, div_min);
}
...
inline void positive_float_sort_rec(RandomAccessIter first,
		RandomAccessIter last, std::vector<RandomAccessIter> &bin_cache,
	...
	if (max == min)
		return;
	unsigned log_divisor = get_log_divisor(last - first,
			rough_log_2_size((size_t) (max) - min));
	div_type div_min = min >> log_divisor;
	div_type div_max = max >> log_divisor;
	unsigned bin_count = div_max - div_min + 1;
	unsigned cache_end;
	...
	//Calculating the size of each bin
	for (RandomAccessIter current = first; current != last;)
		bin_sizes[(cast_float_iter<div_type, RandomAccessIter>(current++)
				>> log_divisor) - div_min]++;
	bins[0] = first;
	for (unsigned u = 0; u < bin_count - 1; u++)
		bins[u + 1] = bins[u] + bin_sizes[u];
	...
	RandomAccessIter nextbinstart = first;
	for (unsigned u = 0; u < bin_count - 1; ++u)
		float_swap_loop<RandomAccessIter, div_type, data_type>(bins,
				nextbinstart, u, bin_sizes, log_divisor, div_min);
	bins[bin_count - 1] = last;
	...
	for (unsigned u = cache_offset; u < cache_end;
		...
		if (count < max_count)
			...
			positive_float_sort_rec<RandomAccessIter, div_type, data_type>(
					lastPos, bin_cache[u], bin_cache, cache_end, bin_sizes);
	}
}
...
inline void negative_float_sort_rec(RandomAccessIter first,
	...
	for (int ii = bin_count - 1; ii > 0; --ii)
		float_swap_loop<RandomAccessIter, div_type, data_type>(bins,
				nextbinstart, ii, bin_sizes, log_divisor, div_min);
	...
	size_t max_count = get_max_count(log_divisor, last - first);
	RandomAccessIter lastPos = first;
	for (int ii = cache_end - 1; ii >= (int) cache_offset; lastPos =
			bin_cache[ii], --ii) {
		...
		if (count < max_count)
			...
			negative_float_sort_rec<RandomAccessIter, div_type, data_type>(
...
inline void negative_float_sort_rec(RandomAccessIter first,
	...
	for (int ii = cache_end - 1; ii >= (int) cache_offset; lastPos =
		...
		if (count < max_count)
			...
			negative_float_sort_rec<RandomAccessIter, div_type, data_type,
					right_shift, compare>(lastPos, bin_cache[ii], bin_cache,
					cache_end, bin_sizes, shift, comp);
	}
}
...
inline void float_sort_rec(RandomAccessIter first, RandomAccessIter last,
	...
	for (unsigned u = 0; u < bin_count; ++u) {
		...
		inner_float_swap_loop<RandomAccessIter, div_type, data_type>(bins,
				nextbinstart, u, log_divisor, div_min);
	}
	...
	for (unsigned u = cache_offset + first_positive; u < cache_end; lastPos =
			bin_cache[u], ++u) {
		size_t count = bin_cache[u] - lastPos;
		if (count < 2)
			continue;
...
inline void float_sort_rec(RandomAccessIter first, RandomAccessIter last,
	...
	for (unsigned u = 0; u < bin_count; ++u) {
		nextbinstart = first + bin_sizes[u];
		inner_swap_loop<RandomAccessIter, div_type, data_type, right_shift>(
				bins, nextbinstart, u, shift, log_divisor, div_min);
	}
...
inline void float_Sort(RandomAccessIter first, RandomAccessIter last, cast_type,
		data_type) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	float_sort_rec<RandomAccessIter, cast_type, data_type>(first, last,
			bin_cache, 0, bin_sizes);
}

template<class RandomAccessIter, class div_type, class data_type,
		class right_shift>
inline void float_Sort(RandomAccessIter first, RandomAccessIter last, div_type,
		data_type, right_shift shift) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	float_sort_rec<RandomAccessIter, div_type, data_type, right_shift>(first,
			last, bin_cache, 0, bin_sizes, shift);
}
...
inline void float_Sort(RandomAccessIter first, RandomAccessIter last, div_type,
	...
	std::vector<RandomAccessIter> bin_cache;
	float_sort_rec<RandomAccessIter, div_type, data_type, right_shift>(first,
			last, bin_cache, 0, bin_sizes, shift, comp);
}
}
...
template<class RandomAccessIter, class cast_type>
inline void float_sort_cast(RandomAccessIter first, RandomAccessIter last,
		cast_type cVal) {
	if (last - first < detail::MIN_SORT_SIZE)
		std::sort(first, last);
	else
...
template<class RandomAccessIter>
inline void float_sort_cast_to_int(RandomAccessIter first,
	...
	while (!done) {
		...
		do {
			...
			if ((*curr).size() > char_offset
					&& ((*curr).size() <= (nextOffset + 1)
							|| (*curr)[nextOffset] != (*first)[nextOffset])) {
				done = true;
				break;
			}
		} while (++curr != finish);
		...
		do {
			...
			if (length(*curr) > char_offset
					&& (length(*curr) <= (nextOffset + 1)
							|| getchar((*curr), nextOffset)
									!= getchar((*first), nextOffset))) {
				done = true;
				break;
			}
		} while (++curr != finish);
...
struct offset_lessthan {
	offset_lessthan(unsigned char_offset) :
			fchar_offset(char_offset) {
	}
	inline bool operator()(const data_type &x, const data_type &y) const {
		unsigned minSize = std::min(x.size(), y.size());
		for (unsigned u = fchar_offset; u < minSize; ++u) {
			if (static_cast<unsignedchar_type>(x[u])
					< static_cast<unsignedchar_type>(y[u]))
				return true;
			else if (static_cast<unsignedchar_type>(y[u])
					< static_cast<unsignedchar_type>(x[u]))
				return false;
		}
...
struct offset_greaterthan {
	offset_greaterthan(unsigned char_offset) :
			fchar_offset(char_offset) {
	}
	inline bool operator()(const data_type &x, const data_type &y) const {
		unsigned minSize = std::min(x.size(), y.size());
...
struct offset_char_lessthan {
	offset_char_lessthan(unsigned char_offset) :
			fchar_offset(char_offset) {
	}
	inline bool operator()(const data_type &x, const data_type &y) const {
...
inline void string_sort_rec(RandomAccessIter first, RandomAccessIter last,
	...
	for (RandomAccessIter current = first; current != last; ++current) {
		if ((*current).size() <= char_offset) {
			...
			bin_sizes[static_cast<unsignedchar_type>((*current)[char_offset])
					+ 1]++;
	}
	//Assign the bin positions
	bin_cache[cache_offset] = first;
	for (unsigned u = 0; u < membin_count - 1; u++)
		bin_cache[cache_offset + u + 1] = bin_cache[cache_offset + u]
				+ bin_sizes[u];
	...
	for (RandomAccessIter current = *local_bin; current < nextbinstart;
		...
		while ((*current).size() > char_offset) {
			target_bin = bins
					+ static_cast<unsignedchar_type>((*current)[char_offset]);
	...
	*local_bin = nextbinstart;
	...
	for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2;
		...
		if (count < max_size)
			std::sort(lastPos, bin_cache[u],
					offset_lessthan<data_type, unsignedchar_type>(
							char_offset + 1));
		else
			string_sort_rec<RandomAccessIter, data_type, unsignedchar_type>(
					lastPos, bin_cache[u], char_offset + 1, bin_cache,
					cache_end, bin_sizes);
	}
}
...
inline void reverse_string_sort_rec(RandomAccessIter first,
		RandomAccessIter last, unsigned char_offset,
		std::vector<RandomAccessIter> &bin_cache, unsigned cache_offset,
		std::vector<size_t> &bin_sizes) {
	//This section is not strictly necessary, but makes handling of long identical substrings much faster, with a mild average performance impact.
	RandomAccessIter curr = first;
	...
	*local_bin = nextbinstart;
	...
	RandomAccessIter lastPos = first;
	//Skip this loop for empties
	for (unsigned u = cache_offset; u <= cache_offset + last_bin; lastPos =
			bin_cache[u], ++u) {
		size_t count = bin_cache[u] - lastPos;
		//don't sort unless there are at least two items to compare
		if (count < 2)
		...
		if (count < max_size)
			std::sort(lastPos, bin_cache[u],
					offset_greaterthan<data_type, unsignedchar_type>(
							char_offset + 1));
		else
			reverse_string_sort_rec<RandomAccessIter, data_type,
					unsignedchar_type>(lastPos, bin_cache[u], char_offset + 1,
...
//String sorting recursive implementation
template<class RandomAccessIter, class data_type, class unsignedchar_type,
		class get_char, class get_length>
inline void string_sort_rec(RandomAccessIter first, RandomAccessIter last,
		unsigned char_offset, std::vector<RandomAccessIter> &bin_cache,
		unsigned cache_offset, std::vector<size_t> &bin_sizes, get_char getchar,
		get_length length) {
	//This section is not strictly necessary, but makes handling of long identical substrings much faster, with a mild average performance impact.
	//Iterate to the end of the empties.  If all empty, return
	...
	*local_bin = nextbinstart;
	...
	for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2;
		...
		if (count < max_size)
			std::sort(lastPos, bin_cache[u],
					offset_char_lessthan<data_type, get_char, get_length>(
							char_offset + 1));
		else
			string_sort_rec<RandomAccessIter, data_type, unsignedchar_type,
					get_char, get_length>(lastPos, bin_cache[u],
					char_offset + 1, bin_cache, cache_end, bin_sizes, getchar,
					length);
	}
}
...
//String sorting recursive implementation
template<class RandomAccessIter, class data_type, class unsignedchar_type,
		class get_char, class get_length, class compare>
inline void string_sort_rec(RandomAccessIter first, RandomAccessIter last,
		unsigned char_offset, std::vector<RandomAccessIter> &bin_cache,
		unsigned cache_offset, std::vector<size_t> &bin_sizes, get_char getchar,
	...
	*local_bin = nextbinstart;
	...
	for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2;
		...
		if (count < max_size)
			...
			string_sort_rec<RandomAccessIter, data_type, unsignedchar_type,
					get_char, get_length, compare>(lastPos, bin_cache[u],
					char_offset + 1, bin_cache, cache_end, bin_sizes, getchar,
					length, comp);
	}
}
...
inline void reverse_string_sort_rec(RandomAccessIter first,
		RandomAccessIter last, unsigned char_offset,
		std::vector<RandomAccessIter> &bin_cache, unsigned cache_offset,
		std::vector<size_t> &bin_sizes, get_char getchar, get_length length,
		compare comp) {
	//This section is not strictly necessary, but makes handling of long identical substrings much faster, with a mild average performance impact.
	RandomAccessIter curr = first;
	//Iterate to the end of the empties.  If all empty, return
	...
	*local_bin = nextbinstart;
	...
	for (unsigned u = cache_offset; u <= cache_offset + last_bin; lastPos =
		...
		if (count < max_size)
			...
			reverse_string_sort_rec<RandomAccessIter, data_type,
					unsignedchar_type, get_char, get_length, compare>(lastPos,
					bin_cache[u], char_offset + 1, bin_cache, cache_end,
...
inline void string_sort(RandomAccessIter first, RandomAccessIter last,
		data_type, unsignedchar_type) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	string_sort_rec<RandomAccessIter, data_type, unsignedchar_type>(first, last,
			0, bin_cache, 0, bin_sizes);
}
...
inline void reverse_string_sort(RandomAccessIter first, RandomAccessIter last,
	...
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	reverse_string_sort_rec<RandomAccessIter, data_type, unsignedchar_type>(
			first, last, 0, bin_cache, 0, bin_sizes);
}

//Holds the bin vector and makes the initial recursive call
template<class RandomAccessIter, class get_char, class get_length,
		class data_type, class unsignedchar_type>
inline void string_sort(RandomAccessIter first, RandomAccessIter last,
		get_char getchar, get_length length, data_type, unsignedchar_type) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char,
			get_length>(first, last, 0, bin_cache, 0, bin_sizes, getchar,
			length);
}

//Holds the bin vector and makes the initial recursive call
template<class RandomAccessIter, class get_char, class get_length,
		class compare, class data_type, class unsignedchar_type>
inline void string_sort(RandomAccessIter first, RandomAccessIter last,
		get_char getchar, get_length length, compare comp, data_type,
		unsignedchar_type) {
	std::vector<size_t> bin_sizes;
	std::vector<RandomAccessIter> bin_cache;
	string_sort_rec<RandomAccessIter, data_type, unsignedchar_type, get_char,
			get_length, compare>(first, last, 0, bin_cache, 0, bin_sizes,
			getchar, length, comp);
}
...
inline void reverse_string_sort(RandomAccessIter first, RandomAccessIter last,
	...
	std::vector<RandomAccessIter> bin_cache;
	reverse_string_sort_rec<RandomAccessIter, data_type, unsignedchar_type,
			get_char, get_length, compare>(first, last, 0, bin_cache, 0,
			bin_sizes, getchar, length, comp);
}
}
...
inline void string_sort(RandomAccessIter first, RandomAccessIter last,
	...
	if (last - first < detail::MIN_SORT_SIZE)
		...
		detail::string_sort(first, last, getchar, length, *first,
				getchar((*first), 0));
	}
}
...
inline void string_sort(RandomAccessIter first, RandomAccessIter last,
	...
	if (last - first < detail::MIN_SORT_SIZE)
		...
		detail::string_sort(first, last, getchar, length, comp, *first,
				getchar((*first), 0));
	}
}
...
inline void reverse_string_sort(RandomAccessIter first, RandomAccessIter last,
	...
	if (last - first < detail::MIN_SORT_SIZE)
		...
		++last;
		detail::reverse_string_sort(first, last, getchar, length, comp, *first,
				getchar((*first), 0));
	}
}
}
