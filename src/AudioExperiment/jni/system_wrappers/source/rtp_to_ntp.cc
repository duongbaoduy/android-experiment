...
// timestamps.
bool CalculateFrequency(int64_t rtcp_ntp_ms1, uint32_t rtp_timestamp1,
		int64_t rtcp_ntp_ms2, uint32_t rtp_timestamp2, double* frequency_khz) {
...
bool CompensateForWrapAround(uint32_t new_timestamp, uint32_t old_timestamp,
...
bool UpdateRtcpList(uint32_t ntp_secs, uint32_t ntp_frac,
		uint32_t rtp_timestamp, RtcpList* rtcp_list, bool* new_rtcp_sr) {
	*new_rtcp_sr = false;
	...
	for (RtcpList::iterator it = rtcp_list->begin(); it != rtcp_list->end();
			++it) {
...
bool RtpToNtpMs(int64_t rtp_timestamp, const RtcpList& rtcp,
	...
	if (!CompensateForWrapAround(rtcp_timestamp_new, rtcp_timestamp_old,
			&rtcp_timestamp_new)) {
		return false;
	}
	double freq_khz;
	if (!CalculateFrequency(rtcp_ntp_ms_new, rtcp_timestamp_new,
			rtcp_ntp_ms_old, rtcp_timestamp_old, &freq_khz)) {
		return false;
	}
