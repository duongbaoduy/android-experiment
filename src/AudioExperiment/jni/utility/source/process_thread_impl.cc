...
#include "utility/source/process_thread_impl.h"
...
ProcessThreadImpl::ProcessThreadImpl() :
		_timeEvent(*EventWrapper::Create()), _critSectModules(
				CriticalSectionWrapper::CreateCriticalSection()), _thread(NULL) {
}
...
int32_t ProcessThreadImpl::RegisterModule(Module* module) {
	...
	for (ModuleList::iterator iter = _modules.begin(); iter != _modules.end();
			++iter) {
		if (module == *iter) {
			return -1;
		}
