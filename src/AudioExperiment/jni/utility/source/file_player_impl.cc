...
#include "utility/source/file_player_impl.h"
...
FilePlayerImpl::FilePlayerImpl(const uint32_t instanceID,
		...
		_instanceID(instanceID), _fileFormat(fileFormat), _fileModule(
				*MediaFile::CreateMediaFile(instanceID)), _decodedLengthInMS(0), _audioDecoder(
				instanceID), _codec(), _numberOf10MsPerFrame(0), _numberOf10MsInDecoder(
				0), _resampler(), _scaling(1.0) {
	_codec.plfreq = 0;
}
...
int32_t FilePlayerImpl::Get10msAudioFromFile(int16_t* outBuffer,
		int& lengthInSamples, int frequencyInHz) {
	if (_codec.plfreq == 0) {
		LOG(LS_WARNING)
				<< "Get10msAudioFromFile() playing not started!"
	...
	if (STR_CASE_CMP(_codec.plname, "L16") == 0) {
		...
		unresampledAudioFrame.samples_per_channel_ = (uint16_t) lengthInBytes
				>> 1;
		...
		if (_audioDecoder.Decode(unresampledAudioFrame, frequencyInHz,
				(int8_t*) encodedBuffer, encodedLengthInBytes) == -1) {
			return -1;
		}
	}
...
int32_t FilePlayerImpl::StartPlayingFile(const char* fileName, bool loop,
		uint32_t startPosition, float volumeScaling, uint32_t notification,
		uint32_t stopPosition, const CodecInst* codecInst) {
	if (_fileFormat == kFileFormatPcm16kHzFile
		...
		if (_fileFormat == kFileFormatPcm8kHzFile) {
			...
			LOG(LS_ERROR)
					<< "StartPlayingFile() sample frequency not "
							<< "supported for PCM format.";
			return -1;
		}

		if (_fileModule.StartPlayingAudioFile(fileName, notification, loop,
				_fileFormat, &codecInstL16, startPosition, stopPosition)
				== -1) {
			LOG(LS_WARNING)
					<< "StartPlayingFile() failed to initialize " << "pcm file "
							<< fileName;
			return -1;
		}
		...
		if (_fileModule.StartPlayingAudioFile(fileName, notification, loop,
			...
			LOG(LS_WARNING)
					<< "StartPlayingFile() failed to initialize "
		...
		if (_fileModule.StartPlayingAudioFile(fileName, notification, loop,
				_fileFormat, no_inst, startPosition, stopPosition) == -1) {
			LOG(LS_WARNING)
					<< "StartPlayingFile() failed to initialize file "
							<< fileName;
			return -1;
		}
		SetAudioScaling(volumeScaling);
...
int32_t FilePlayerImpl::StartPlayingFile(InStream& sourceStream,
	...
	if (_fileFormat == kFileFormatPcm16kHzFile
		...
		if (_fileModule.StartPlayingAudioStream(sourceStream, notification,
				...
				== -1) {
			LOG(LS_ERROR)
					<< "StartPlayingFile() failed to initialize stream "
							<< "playout.";
			return -1;
		}
...
int32_t FilePlayerImpl::SetUpAudioDecoder() {
	...
	if (STR_CASE_CMP(_codec.plname, "L16") != 0
		...
		LOG(LS_WARNING)
				<< "SetUpAudioDecoder() codec " << _codec.plname
						<< " not supported.";
		return -1;
	}
