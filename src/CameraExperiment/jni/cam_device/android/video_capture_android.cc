...
#include "cam_device/android/video_capture_android.h"
#include "cam_device/include/video_capture.h"
#include <assert.h>

#include "base/common.h"
#include "utility/interface/helpers_android.h"
#include "cam_device/android/device_info_android.h"
...
void JNICALL ProvideCameraFrame(JNIEnv* env, jobject,
		jbyteArray javaCameraFrame, jint length, jint rotation, jlong timeStamp,
		jlong context) {
	webrtc::videocapturemodule::VideoCaptureAndroid* captureModule =
			reinterpret_cast<webrtc::videocapturemodule::VideoCaptureAndroid*>(context);
	jbyte* cameraFrame = env->GetByteArrayElements(javaCameraFrame, NULL);
	captureModule->OnIncomingFrame(reinterpret_cast<uint8_t*>(cameraFrame),
			length, rotation, 0);
...
int32_t SetCaptureAndroidVM(JavaVM* javaVM, jobject context) {
	if (javaVM) {
		...
		jclass j_capture_class = ats.env()->FindClass(
				"org/webrtc/videoengine/VideoCaptureAndroid");
		assert(j_capture_class);
		g_java_capturer_class =
				reinterpret_cast<jclass>(ats.env()->NewGlobalRef(
						j_capture_class));
		assert(g_java_capturer_class);

		JNINativeMethod native_methods[] = { { "GetContext",
				"()Landroid/content/Context;",
				reinterpret_cast<void*>(&GetContext) }, { "ProvideCameraFrame",
				"([BIIJJ)V", reinterpret_cast<void*>(&ProvideCameraFrame) } };
		if (ats.env()->RegisterNatives(g_java_capturer_class, native_methods, 2)
				!= 0)
			assert(false);
	} else {
		if (g_jvm) {
...
int32_t VideoCaptureAndroid::OnIncomingFrame(uint8_t* videoFrame,
		size_t videoFrameLength, int32_t degrees, int64_t captureTime) {
	if (!_captureStarted)
		return 0;
	VideoCaptureRotation current_rotation =
			...
			(degrees > 225 && degrees <= 315) ?
					kCameraRotate270 : kCameraRotate0; // Impossible.
	...
	return IncomingFrame(videoFrame, videoFrameLength, _captureCapability,
			captureTime);
}

VideoCaptureAndroid::VideoCaptureAndroid(const int32_t id) :
		VideoCaptureImpl(id), _deviceInfo(id), _jCapturer(NULL), _captureStarted(
				false) {
}
...
int32_t VideoCaptureAndroid::StartCapture(
	...
	if (_deviceInfo.GetBestMatchedCapability(_deviceUniqueId, capability,
		...
		WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, -1,
				"%s: GetBestMatchedCapability failed: %dx%d", __FUNCTION__,
	...
	jmethodID j_start = env->GetMethodID(g_java_capturer_class, "startCapture",
			"(IIII)Z");
	assert(j_start);
	int min_mfps = 0;
	int max_mfps = 0;
	...
	bool started = env->CallBooleanMethod(_jCapturer, j_start,
			_captureCapability.width, _captureCapability.height, min_mfps,
			max_mfps);
	if (started) {
...
int32_t VideoCaptureAndroid::StopCapture() {
	...
	jmethodID j_stop = env->GetMethodID(g_java_capturer_class, "stopCapture",
			"()Z");
...
int32_t VideoCaptureAndroid::SetCaptureRotation(VideoCaptureRotation rotation) {
	...
	jmethodID j_spr = env->GetMethodID(g_java_capturer_class,
			"setPreviewRotation", "(I)V");
	assert(j_spr);
