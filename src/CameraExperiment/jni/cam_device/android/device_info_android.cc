...
struct AndroidCameraInfo {
	...
	std::string ToString() {
		...
		stream << "Name: [" << name << "], MFPS ranges: ["
				<< IntPairsToString(mfpsRanges, ':') << "], front_facing: "
				<< front_facing << ", orientation: " << orientation
				<< ", resolutions: [" << IntPairsToString(resolutions, 'x')
				<< "]";
		return stream.str();
	}
};
...
void DeviceInfoAndroid::Initialize(JNIEnv* jni) {
	...
	jclass j_info_class = jni->FindClass(
			"org/webrtc/videoengine/VideoCaptureDeviceInfoAndroid");
	assert(j_info_class);
	jmethodID j_initialize = jni->GetStaticMethodID(j_info_class,
			"getDeviceInfo", "()Ljava/lang/String;");
	jstring j_json_info = static_cast<jstring>(jni->CallStaticObjectMethod(
			j_info_class, j_initialize));
	...
	for (Json::ArrayIndex i = 0; i < cameras.size(); ++i) {
		...
		for (Json::ArrayIndex j = 0; j < sizes.size(); ++j) {
			...
			info.resolutions.push_back(
					std::make_pair(size["width"].asInt(),
							size["height"].asInt()));
		}
		...
		for (Json::ArrayIndex j = 0; j < mfpsRanges.size(); ++j) {
			...
			info.mfpsRanges.push_back(
					std::make_pair(mfpsRange["min_mfps"].asInt(),
...
void DeviceInfoAndroid::GetMFpsRange(const char* deviceUniqueIdUTF8,
		int max_fps_to_match, int* min_mfps, int* max_mfps) {
	const AndroidCameraInfo* info = FindCameraInfoByName(deviceUniqueIdUTF8);
	if (info == NULL)
		return;
