...
#include "cam_device/device_info_impl.h"
#include "cam_device/video_capture_config.h"
...
DeviceInfoImpl::DeviceInfoImpl(const int32_t id) :
		_id(id), _apiLock(*RWLockWrapper::CreateRWLock()), _lastUsedDeviceName(
				NULL), _lastUsedDeviceNameLength(0) {
}
...
int32_t DeviceInfoImpl::GetCapability(const char* deviceUniqueIdUTF8,
	...
	if ((_lastUsedDeviceNameLength != strlen((char*) deviceUniqueIdUTF8))
			...
			|| (strncasecmp((char*) _lastUsedDeviceName,
					(char*) deviceUniqueIdUTF8, _lastUsedDeviceNameLength) != 0))
#else
	...
	if (deviceCapabilityNumber >= (unsigned int) _captureCapabilities.size()) {
		LOG(LS_ERROR)
				<< "Invalid deviceCapabilityNumber " << deviceCapabilityNumber
...
int32_t DeviceInfoImpl::GetBestMatchedCapability(const char*deviceUniqueIdUTF8,
	...
	for (int32_t tmp = 0; tmp < numberOfCapabilies; ++tmp) // Loop through all capabilities
		...
		const int32_t currentbestDiffFrameRate = bestFrameRate
				- requested.maxFPS;
					...
					if (diffWidth == currentbestDiffWith
						...
						if (((diffFrameRate >= 0
								...
								|| (currentbestDiffFrameRate < 0
										&& diffFrameRate
												>= currentbestDiffFrameRate)) // Current frame rate is lower than requested. This is better.
						) {
	...
	LOG(LS_VERBOSE)
			<< "Best camera format: " << bestWidth << "x" << bestHeight << "@"
					<< bestFrameRate << "fps, color format: " << bestRawType;
...
int32_t DeviceInfoImpl::GetExpectedCaptureDelay(const DelayValues delayValues[],
		const uint32_t sizeOfDelayValues, const char* productId,
		const uint32_t width, const uint32_t height) {
	int32_t bestDelay = kDefaultCaptureDelay;

	for (uint32_t device = 0; device < sizeOfDelayValues; ++device) {
		if (delayValues[device].productId
				&& strncmp((char*) productId,
			...
			for (uint32_t delayIndex = 0; delayIndex < NoOfDelayValues;
					++delayIndex) {
				const DelayValue& currentValue =
						delayValues[device].delayValues[delayIndex];
					...
					if (diffHeight == currentbestDiffHeight) // Found best height. Care about the width)
						...
						if ((diffWidth >= 0
								...
								|| (currentbestDiffWith < 0
										&& diffWidth >= currentbestDiffWith)) {
							if (diffWidth == currentbestDiffWith
									&& diffHeight == currentbestDiffHeight) // Same size as previous
											{
							} else // Better width than previously
							{
