...
// Statistics for RTCP packet types.
	struct RtcpPacketTypeCounter {
			...
			void Add(
			         const RtcpPacketTypeCounter& other) {
			...
			int64_t TimeSinceFirstPacketInMs(
			...
			int UniqueNackRequestsInPercent() const {
				...
				return static_cast<int>((unique_nack_requests * 100.0f
				        / nack_requests) + 0.5f);
			}
...
// Data usage statistics for a (rtp) stream.
	struct StreamDataCounters {
			...
			void Add(
			         const StreamDataCounters& other) {
				bytes += other.bytes;
				...
				retransmitted_padding_bytes +=
				        other.retransmitted_padding_bytes;
...
// Callback, called whenever byte/packet counts have been updated.
	class StreamDataCountersCallback {
		public:
			...
			virtual void DataCountersUpdated(
...
// Callback, used to notify an observer whenever new rates have been estimated.
	class BitrateStatisticsObserver {
		public:
			...
			virtual void Notify(
			                    const BitrateStatistics& total_stats,
...
// Callback, used to notify an observer whenever frame counts have been updated.
	class FrameCountObserver {
		public:
			...
			virtual void FrameCountUpdated(
...
// Callback, used to notify an observer whenever the send-side delay is updated.
	class SendSideDelayObserver {
		public:
			...
			virtual void SendSideDelayUpdated(
			                                  int avg_delay_ms,
			                                  int max_delay_ms,
			                                  uint32_t ssrc) = 0;
	};
...
// RTP
	...
	enum RTPDirections {
		kRtpIncoming = 0, kRtpOutgoing
	};

	enum PayloadFrequencies {
		kFreq8000Hz = 8000, kFreq16000Hz = 16000, kFreq32000Hz = 32000
	};
...
// AGC configuration
	...
	enum StereoChannel {
		kStereoLeft = 0, kStereoRight, kStereoBoth
	};
...
// Video codec
	...
	enum VideoCodecProfile {
		kProfileBase = 0x00, kProfileMain = 0x01
	};
...
// settings such as resolution.
	struct SimulcastStream {
			...
			bool operator==(
			                const SimulcastStream& other) const {
...
// Common video codec properties
	struct VideoCodec {
			...
			bool operator==(
			                const VideoCodec& other) const {
				bool ret = codecType == other.codecType
				        && (STR_CASE_CMP(plName, other.plName) == 0)
				        && plType == other.plType && width == other.width
				        && height == other.height
				...
				for (unsigned char i = 0;
				        i < other.numberOfSimulcastStreams && ret; ++i) {
