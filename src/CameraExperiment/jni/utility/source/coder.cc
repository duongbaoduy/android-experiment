...
#include "utility/source/coder.h"

namespace webrtc {
AudioCoder::AudioCoder(uint32_t instanceID) :
		_acm(AudioCodingModule::Create(instanceID)), _receiveCodec(), _encodeTimestamp(
				0), _encodedData(NULL), _encodedLengthInBytes(0), _decodeTimestamp(
				0) {
...
int32_t AudioCoder::Decode(AudioFrame& decodedAudio, uint32_t sampFreqHz,
	...
	if (payloadLength > 0) {
		...
		if (_acm->IncomingPayload((const uint8_t*) incomingPayload,
				payloadLength, payloadType, _decodeTimestamp) == -1) {
			return -1;
		}
	}
...
int32_t AudioCoder::Encode(const AudioFrame& audio, int8_t* encodedData,
