...
ThreadPosix::ThreadPosix(ThreadRunFunction func, ThreadObj obj,
		ThreadPriority prio, const char* thread_name) :
		run_function_(func), obj_(obj), crit_state_(
				CriticalSectionWrapper::CreateCriticalSection()), alive_(false), dead_(
				true), prio_(prio), event_(EventWrapper::Create()), name_(), set_thread_name_(
				false),
...
void ThreadPosix::Run() {
	...
	if (max_prio - min_prio > 2) {
		sched_param param;
		param.sched_priority = ConvertToSystemPriority(prio_, min_prio,
				max_prio);
