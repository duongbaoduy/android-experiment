...
#define THIS_FILE "trace_impl.cc"
...
TraceImpl* TraceImpl::StaticInstance(CountOperation count_operation,
	...
	TraceImpl* impl = GetStaticInstance < TraceImpl > (count_operation);
	return impl;
}
...
TraceImpl::TraceImpl() :
		critsect_interface_(CriticalSectionWrapper::CreateCriticalSection()), callback_(
				NULL), row_count_text_(0), file_count_text_(0), trace_file_(
				*FileWrapper::Create()), thread_(
				*ThreadWrapper::CreateThread(TraceImpl::Run, this,
						kHighestPriority, "Trace")), event_(
				*EventWrapper::Create()), critsect_array_(
				CriticalSectionWrapper::CreateCriticalSection()), next_free_idx_(), level_(), length_(), message_queue_(), active_queue_(
				0) {
	next_free_idx_[0] = 0;
	next_free_idx_[1] = 0;
...
int32_t TraceImpl::AddModuleAndId(char* trace_message, const TraceModule module,
...
int32_t TraceImpl::SetTraceFileImpl(const char* file_name_utf8,
	...
	if (file_name_utf8) {
		if (add_file_counter) {
			...
			file_count_text_ = 0;
			if (trace_file_.OpenFile(file_name_utf8, false, false, true)
					== -1) {
				return -1;
			}
		}
...
int32_t TraceImpl::AddMessage(char* trace_message,
	...
	length = snprintf(trace_message,
			WEBRTC_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2, "%s", msg);
	if (length < 0
			|| length > WEBRTC_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2) {
		length = WEBRTC_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2;
		trace_message[length] = 0;
...
void TraceImpl::AddMessageToList(
		...
		const uint16_t length, const TraceLevel level) {
// NOTE(andresp): Enabled externally.
	...
	if (next_free_idx_[active_queue_] == WEBRTC_TRACE_MAX_QUEUE - 1) {
		...
		length_[active_queue_][next_free_idx_[active_queue_]] = strlen(
				warning_msg);
...
void TraceImpl::WriteToFile() {
	...
	for (uint16_t idx = 0; idx < local_next_free_idx; ++idx) {
		...
		if (callback_) {
			callback_->Print(local_level,
					message_queue_[local_queue_active][idx],
		...
		if (trace_file_.Open()) {
			if (row_count_text_ > WEBRTC_TRACE_MAX_FILE_SIZE) {
				...
				if (file_count_text_ == 0) {
					...
					UpdateFileName(old_file_name, new_file_name,
							file_count_text_);

					if (trace_file_.OpenFile(new_file_name, false, false, true)
							== -1) {
						return;
					}
				}
...
void TraceImpl::AddImpl(const TraceLevel level, const TraceModule module,
		const int32_t id, const char msg[WEBRTC_TRACE_MAX_MESSAGE_SIZE]) {
	if (TraceCheck(level)) {
...
#else
				vsnprintf(temp_buff, WEBRTC_TRACE_MAX_MESSAGE_SIZE - 1, msg,
						args);
#endif
				va_end(args);
				buff = temp_buff;
...
class LogCallback: public TraceCallback {
public:
	void Print(const TraceLevel level, const char *traceString,
			const int length) {
		if (traceString) {
			__android_log_print(ANDROID_LOG_ERROR, THIS_FILE, "%s",
					traceString);
		}
	}
	LogCallback() {
...
static webrtc::LogCallback trace;

void webrtc_trace_create() {
	webrtc::Trace::CreateTrace();

	webrtc::Trace::set_level_filter(webrtc::kTraceAll);
	webrtc::Trace::SetTraceCallback(&trace);
}
