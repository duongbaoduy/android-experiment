...
Row::Row() :
		cells_(), cells_lock_(CriticalSectionWrapper::CreateCriticalSection()) {
}
...
void Row::ToString(const std::string& column_name, std::string* value_string) {
	CriticalSectionScoped synchronize(cells_lock_);
...
LogTable::LogTable() :
		columns_(), rows_(), rows_history_(&rows_[0]), rows_flush_(&rows_[1]), current_row_(
				new Row), file_(FileWrapper::Create()), write_header_(true), table_lock_(
				CriticalSectionWrapper::CreateCriticalSection()) {
}
...
int LogTable::CreateLogFile(const std::string& file_name) {
	...
	if (file_->Open())
		return -1;
	file_->OpenFile(file_name.c_str(), false, // Open with read/write permissions
...
void LogTable::Flush() {
	...
	if (commit_header) {
		for (column_it = columns_.begin(); column_it != columns_.end();
				++column_it) {
...
int DataLog::AddColumn(const std::string& table_name,
		const std::string& column_name, int multi_value_length) {
	DataLogImpl* data_log = DataLogImpl::StaticInstance();
	if (data_log == NULL)
		return -1;
	return data_log->DataLogImpl::StaticInstance()->AddColumn(table_name,
			column_name, multi_value_length);
}
...
DataLogImpl::DataLogImpl() :
		counter_(1), tables_(), flush_event_(EventWrapper::Create()), file_writer_thread_(
				NULL), tables_lock_(RWLockWrapper::CreateRWLock()) {
}
...
int DataLogImpl::Init() {
	file_writer_thread_ = ThreadWrapper::CreateThread(DataLogImpl::Run,
			instance_, kHighestPriority, "DataLog");
...
int DataLogImpl::InsertCell(const std::string& table_name,
		const std::string& column_name, const Container* value_container) {
	ReadLockScoped synchronize(*tables_lock_);
...
void DataLogImpl::StopThread() {
	if (file_writer_thread_ != NULL) {
		...
		// finish.
		while (!file_writer_thread_->Stop())
			continue;
	}
}
