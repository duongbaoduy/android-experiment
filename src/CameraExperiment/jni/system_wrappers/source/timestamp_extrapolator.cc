...
TimestampExtrapolator::TimestampExtrapolator(int64_t start_ms) :
		_rwLock(RWLockWrapper::CreateRWLock()), _startMs(0), _firstTimestamp(0), _wrapArounds(
				0), _prevUnwrappedTimestamp(-1), _prevWrapTimestamp(-1), _lambda(
				1), _firstAfterReset(true), _packetCount(0), _startUpFilterDelayInPackets(
				2), _detectorAccumulatorPos(0), _detectorAccumulatorNeg(0), _alarmThreshold(
				60e3), _accDrift(6600), // in timestamp ticks, i.e. 15 ms
		_accMaxError(7000), _pP11(1e10) {
	Reset(start_ms);
}
...
int64_t TimestampExtrapolator::ExtrapolateLocalTime(uint32_t timestamp90khz) {
	...
	if (_packetCount == 0) {
		...
		localTimeMs = _prevMs
				+ static_cast<int64_t>(static_cast<double>(unwrapped_ts90khz
						- _prevUnwrappedTimestamp) / 90.0 + 0.5);
	} else {
		if (_w[0] < 1e-3) {
...
bool TimestampExtrapolator::DelayChangeDetection(double error) {
	...
	error = (error > 0) ?
			std::min(error, _accMaxError) : std::max(error, -_accMaxError);
	_detectorAccumulatorPos = std::max(
			_detectorAccumulatorPos + error - _accDrift, (double) 0);
	_detectorAccumulatorNeg = std::min(
			_detectorAccumulatorNeg + error + _accDrift, (double) 0);
