...
public:
	...
	virtual int FileName(char* file_name_utf8, size_t size) const OVERRIDE;

	virtual bool Open() const OVERRIDE;

	virtual int OpenFile(const char* file_name_utf8, bool read_only, bool loop =
			false, bool text = false) OVERRIDE;

	virtual int OpenFromFileHandle(FILE* handle, bool manage_file,
			bool read_only, bool loop = false) OVERRIDE;
